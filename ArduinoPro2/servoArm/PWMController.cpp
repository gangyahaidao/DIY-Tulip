...
uint16 ServoPwmDuty[8] = {1500, 1500, 1500, 1500, 1500, 1500, 1500, 1500}; //PWM脉冲宽度，存储上一次转动之后的角度
uint16 ServoPwmDutySet[8] = {1500, 1500, 1500, 1500, 1500, 1500, 1500, 1500}; //PWM脉冲宽度,上位机下发的角度
...
void ServoPwmDutyCompare(void) { //脉宽变化比较及速度控制,每20ms调用一次
  ...
  if (ServoRunning){
    ...
    for (i = 0; i < 8; i++){
      ...
      if ((i >= 0) && (i <= 6)){//依次写入角度值
        myservo[i - 1].writeMicroseconds(ServoPwmDuty[i]);//写入旋转角度，1000~2000,1500是在中间，与write函数一样，只是参数是微妙us
      }
    }
  }
}
